...
local PremiumItems = {
    ["God Human"] = {"God Human", "GodHuman", "God-Human", "GodlyHuman"},
    ["Cursed Dual Katana"] = {"Cursed Dual Katana", "CDK"},
    ["Soul Guitar"] = {"Soul Guitar", "Skull Guitar"},
    ["Mirror Fractal"] = {"Mirror Fractal", "Fractal", "MirrorFractal"},
    ["Valkyrie Helm"] = {"Valkyrie Helm", "VHelm", "Valkyrie_Helm", "Valkyrie"}
}
...
function LionHub:CreateUI()
    ...
    GUI.IgnoreGuiInset = true -- This makes UI cover the topbar area
    ...
    -- Main container frame - FULLSCREEN with proper scaling
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"
    mainFrame.BackgroundColor3 = Config.BackgroundColor
    mainFrame.BackgroundTransparency = Config.BackgroundTransparency
    mainFrame.Size = UDim2.new(1, 0, 1, 0) -- Full screen size
    ...
    -- Content container (adaptive layout for different screen sizes)
    local contentContainer = Instance.new("Frame")
    contentContainer.Name = "ContentContainer"
    contentContainer.BackgroundTransparency = 1
    contentContainer.Size = UDim2.new(0.9, 0, 0.9, 0) -- 90% of screen size
    contentContainer.Position = UDim2.new(0.05, 0, 0.05, 0) -- Centered with 5% padding
    contentContainer.Parent = mainFrame
    
    -- Three-column layout container with adaptive sizing
    local columnsFrame = Instance.new("Frame")
    columnsFrame.Name = "ColumnsFrame"
    columnsFrame.BackgroundTransparency = 1
    columnsFrame.Size = UDim2.new(1, 0, 0.9, 0) -- Use 90% of container height
    columnsFrame.Position = UDim2.new(0, 0, 0.05, 0) -- Center vertically with 5% padding
    columnsFrame.Parent = contentContainer
    
    -- Grid layout for responsive columns
    local gridLayout = Instance.new("UIGridLayout")
    gridLayout.CellPadding = UDim2.new(0.02, 0, 0, 0) -- 2% padding between columns
    gridLayout.CellSize = UDim2.new(0.32, 0, 1, 0) -- ~32% width for each column
    ...
    itemsContent.ScrollBarThickness = 4
    itemsContent.ScrollBarImageColor3 = Config.AccentColor
    itemsContent.CanvasSize = UDim2.new(0, 0, 0, 0) -- Auto-sizing
    ...
    -- Auto-size canvas based on content
    itemsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        itemsContent.CanvasSize = UDim2.new(0, 0, 0, itemsLayout.AbsoluteContentSize.Y)
    end)
    ...
    playerContent.ScrollBarImageColor3 = Config.AccentColor
    playerContent.CanvasSize = UDim2.new(0, 0, 0, 0) -- Auto-sizing
    ...
    playerLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        playerContent.CanvasSize = UDim2.new(0, 0, 0, playerLayout.AbsoluteContentSize.Y)
    end)
    ...
    local questContent = Instance.new("ScrollingFrame")
    ...
    questContent.ScrollBarThickness = 4
    questContent.ScrollBarImageColor3 = Config.AccentColor
    questContent.CanvasSize = UDim2.new(0, 0, 0, 0) -- Auto-sizing
    ...
    questLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        questContent.CanvasSize = UDim2.new(0, 0, 0, questLayout.AbsoluteContentSize.Y)
    end)
    ...
    for i = 1, 2 do
        ...
        separator.Position = UDim2.new(0.33 * i + 0.01, 0, 0.05, 0)
        separator.ZIndex = 2
...
function LionHub:UpdateQuestInfo()
    ...
    local questContent = GUI.MainFrame.ContentContainer.ColumnsFrame.QuestColumn.Content
...
function LionHub:UpdatePlayerInfo()
    ...
    local playerContent = GUI.MainFrame.ContentContainer.ColumnsFrame.PlayerColumn.Content
    local data = LocalPlayer:FindFirstChild("Data")
...
-- IMPROVED ITEM DETECTION: Update premium items with better detection
function LionHub:UpdatePremiumItems()
    if not GUI or not GUI:FindFirstChild("MainFrame") then return end
    
    local itemsContent = GUI.MainFrame.ContentContainer.ColumnsFrame.ItemsColumn.Content
    for mainItemName, _ in pairs(PremiumItems) do
        ...
        if itemFrame then
            ...
            -- Debug print to help troubleshoot item detection
            --print("Checking for item:", mainItemName, "Result:", hasItem)
...
-- FIXED ITEM DETECTION: Improved check for premium items with additional data checks
function LionHub:HasPremiumItem(mainItemName)
    local alternativeNames = PremiumItems[mainItemName] or {mainItemName}
    
    -- Debug function to log item search
    local function debugItem(location, item)
        --print("Checking in", location, ":", item.Name, "Looking for:", table.concat(alternativeNames, ", "))
    end
    ...
    local function itemMatches(item)
        for _, name in ipairs(alternativeNames) do
            if item.Name == name or string.find(item.Name, name, 1, true) then
                return true
            end
        end
        return false
    ...
    -- Check data inventory (multiple levels deep)
    local data = LocalPlayer:FindFirstChild("Data")
    if data then
        -- Direct children first
        if data:FindFirstChild(mainItemName) then
            return true
        end
        ...
        -- Check Items folder if it exists
        local items = data:FindFirstChild("Items")
        if items then
            for _, item in ipairs(items:GetChildren()) do
                debugItem("Items", item)
                if itemMatches(item) then 
                    return true 
                end
            end
        ...
        -- Check WeaponInventory if it exists
        local weaponInventory = data:FindFirstChild("WeaponInventory")
        if weaponInventory then
            for _, item in ipairs(weaponInventory:GetChildren()) do
                debugItem("WeaponInventory", item)
                if itemMatches(item) then 
                    return true 
                end
            end
        ...
        -- Check for properties that might indicate ownership
        for _, alternativeName in ipairs(alternativeNames) do
            if data:FindFirstChild(alternativeName) or 
               data:FindFirstChild(alternativeName .. "Bought") or
               data:FindFirstChild("Owns" .. alternativeName) then
                return true
            end
        end
    end
