...
local PremiumItems = {
    ...
    ["Cursed Dual Katana"] = {"Cursed Dual Katana", "CURSED DUAL KATANA"},
    ["Soul Guitar"] = {"SKULLGUITAR", "Skull Guitar"},
...
function LionHub:CreateUI()
    ...
    mainFrame.Size = UDim2.new(0, 800, 0, 170)
    ...
    playerLayout.Padding = UDim.new(0, 12) -- Tăng khoảng cách giữa các dòng
    ...
    questLayout.Padding = UDim.new(0, 12) -- Tăng khoảng cách giữa các dòng
...
-- Update premium items - IMPROVED TO CHECK ALTERNATIVE NAMES & SPECIAL FOLDERS
...
-- Check for premium item WITH ALTERNATIVE NAMES & SPECIAL FOLDERS
function LionHub:HasPremiumItem(mainItemName)
    ...
    -- Check data inventory & special folders
    local data = LocalPlayer:FindFirstChild("Data")
    if data then
        ...
        -- Special case: God Human, Mirror Fractal, Valkyrie Helm as direct Value
        for _, name in ipairs(alternativeNames) do
